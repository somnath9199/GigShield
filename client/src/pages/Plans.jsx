...
export default function Plans() {
  ...
  const selectPlan = async (plan) => {
    ...
    if (!fleetId) {
      setErr("Fleet ID not found. Please log in again.");
      return;
    }
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/premium/${fleetId}`);
      const result = await response.json();

      if (!response.ok || !result.success) {
        throw new Error(result.message || "Failed to calculate premium");
      }

      const dynamicPrice = result.data.final_price;

      let planAdjustment = 0;
      if (plan.id === "rakshak") planAdjustment = 4;
      if (plan.id === "suraksha") planAdjustment = 10;

      const heatAddon = includeHeat ? plan.heatWeekly : 0;
      const finalWeeklyPremium = dynamicPrice + planAdjustment + heatAddon;
    ...
    <div className="plans-page" style={{ padding: "32px" }}>
      <div className="plans-header" style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 28, fontWeight: 800, color: "#fff", marginBottom: 12 }}>
          Parametric Insurance Plans
        </h1>
        <p style={{ fontSize: 15, color: "var(--muted)", maxWidth: 640, lineHeight: 1.5 }}>
          GigShield collects premium annually to prevent weather-fraud, but we break the pricing
          down into weekly costs so it aligns closely with your regular gig earnings. Select your
          coverage block below to activate your Dashboard.
        </p>
      </div>

      <div
        className="heat-toggle-box"
        style={{
      ...
      >
        <div>
          <h3 style={{ fontSize: 16, color: "#f97316", marginBottom: 4 }}>
            🔥 Extreme Heat Coverage (Optional Add-On)
          </h3>
          <p
            style={{
              fontSize: 13,
          ...
          >
            Add automated payouts for when temperatures exceed 45°C for 3+ consecutive hours in
            your localized zone.
          </p>
        </div>
        ...
        <label
          className="switch"
          style={{ position: "relative", display: "inline-block", width: 50, height: 28, flexShrink: 0 }}
        >
          <input
            type="checkbox"
            checked={includeHeat}
            onChange={(e) => setIncludeHeat(e.target.checked)}
          ...
          <span
            ...
            style={{
              ...
              right: 0,
              bottom: 0,
              backgroundColor: includeHeat ? "#f97316" : "rgba(255,255,255,0.08)",
              transition: ".4s",
              borderRadius: 28,
          ...
          >
            <span
              style={{
                ...
                left: 4,
                bottom: 3,
                backgroundColor: includeHeat ? "#fff" : "#888",
                transition: ".4s",
                borderRadius: "50%",
      ...
      {err && <div className="err-msg" style={{ marginBottom: 20, color: "red" }}>{err}</div>}
        ...
        {PLANS.map((p) => {
          ...
          return (
            ...
            >
              ...
              <h2 style={{ fontSize: 22, fontWeight: 800, color: p.color, marginBottom: 8, letterSpacing: -0.5 }}>
                {p.name}
              </h2>
              <p style={{ fontSize: 13, color: "var(--muted)", minHeight: 38, marginBottom: 20 }}>
                {p.desc}
              </p>
              ...
              <button
                ...
                style={{
                  ...
                  transition: "0.2s",
                  boxShadow: `0 8px 24px ${p.color}44`
                }}
              >
